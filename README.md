...
- **중복 요청 방지**: `Idempotency-Key`가 있는 입출고 요청은 같은 키와 내용의 재호출에 최초 성공 응답을 반환합니다.
...
상품·창고·재고·이력·중복 방지 요청 테이블과 기본 창고(`DEFAULT`)를 생성합니다.
SQL을 직접 실행할 필요가 없습니다.
...
재호출할 가능성이 있는 입출고 요청에는 `Idempotency-Key` 헤더를 보냅니다.
키는 영문·숫자·점·밑줄·콜론·하이픈으로 구성한 1~128자입니다.
새 업무 요청에는 새 키를, 같은 요청의 재시도에는 동일한 키를 사용합니다.

```bash
curl -i -X POST http://localhost:8080/api/products/receipts \
  -H 'Content-Type: application/json' \
  -H 'Idempotency-Key: receipt-example-001' \
  -d '{"name":"상품 A","quantity":10}'
```

같은 키와 내용으로 재호출하면 재고를 다시 변경하지 않고 최초 성공 응답을 반환합니다.
같은 키의 다른 수량·상품·입출고 유형은 `409 IDEMPOTENCY_CONFLICT`로 거부합니다.
헤더를 생략하면 기존처럼 매번 반영됩니다.
신규 요청의 트랜잭션이 롤백되면 해당 요청에서 생성한 키도 롤백되어 같은 키로 다시 시도할 수 있습니다.
이미 성공한 키를 다른 내용으로 재사용하여 409가 발생한 경우에는 기존 키와 응답이 유지됩니다.
서버는 자동 재시도를 수행하지 않습니다.
...
| `idempotency_request` | 중복 방지 키·요청 해시·최초 성공 응답 |
