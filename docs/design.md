...
같은 키가 동시에 들어오면 선행 트랜잭션의 결과를 기다린 뒤 최초 성공 응답을 반환합니다.
다른 내용에 같은 키를 사용하면 `409 IDEMPOTENCY_CONFLICT`를 반환합니다.

요청 키, 응답, 재고 변경, 이력 저장은 모두 같은 트랜잭션에서 확정됩니다.
업무 검증이나 DB 저장이 실패하면 키도 롤백되어 복구 후 같은 키로 다시 시도할 수 있습니다.
